	...
	public static class VoxelFile {
		...
		private static int ReadMaterial (BinaryReader _br, ref VoxelData data) {
			...
			data.Materials.Add(new VoxelData.MaterialData() {
				...
				Weight = TryGetFloat(dic, "_weight", 1f),
				Rough = TryGetFloat(dic, "_rough", 1f),
				Spec = TryGetFloat(dic, "_spec", 1f),
				Ior = TryGetFloat(dic, "_ior", 1f),
				Att = TryGetFloat(dic, "_att", 1f),
				Flux = TryGetFloat(dic, "_flux", 1f),
				Glow = TryGetFloat(dic, "_glow", 1f),
		...
		private static byte[] WriteMaterial (VoxelData.MaterialData material) {
			...
			AddVoxStringBytes(ref bytes, "_glow");
			AddVoxStringBytes(ref bytes, material.Glow.ToString());
